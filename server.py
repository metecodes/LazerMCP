...
@mcp.tool(
    description=(
        ...
        "Optional explicit contour/panel interfaces: part_id plus connectors=[{id,type,part_id,mate_id,position:[x,y,z],axis:[x,y,z],width_mm,depth_mm,thickness_mm}]. IDs are globally unique; mate_id must be reciprocal. position is the local feature volume center (z defaults to half thickness); axis is a local insertion direction. tab/slot and finger/finger_socket bind existing actual CUT features; declarations never draw or move them. They require matching world prisms, collision-free continuous insertion and parameters.assembly_order of physical part IDs. Per-part mates=[{connector_id,target_connector}] is an equivalent declaration. canonical_mates reports per-interface checks, locked mates and errors; failed assemblies have no authorized steps. Other connector type names are reserved and return MATE_GEOMETRY_UNSUPPORTED until their physical adapter exists; keep existing shaft_hole/linear_slide parameters.connections workflows for those mechanisms. "
