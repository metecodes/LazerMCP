...
def build_fixture():
    ...
    static=[side('left chassis',[-3,0,0]),side('right chassis',[50,0,0]),front,rear,
            ...
            {'type':'disc','label':'left front hub','d':10,'hole':4.15,'placement':{'origin':[-5,20,22],'u':[0,1,0],'v':[0,0,1]}},
            {'type':'disc','label':'right front hub','d':10,'hole':4.15,'placement':{'origin':[55,20,22],'u':[0,1,0],'v':[0,0,1]}},
    ...
    parameters={'project':'yol-silindiri-sasi-gecme-v3-reconstructed','hardware_clearance':.15,'hardware':[
        {'id':'front axle','type':'shaft','diameter':4,'hole_diameter':4.15,'axis':[1,0,0],'length':75},
        {'id':'rear drum axle','type':'shaft','diameter':4,'hole_diameter':4.15,'axis':[1,0,0],'length':75}], 'connections':[]}
    for part in rotating:
        parameters['connections'].append({'type':'shaft_hole','shaft':part['mechanism']['shaft'],'part':part['label'],'hole':'center-hole','fit':'rotating','hardware_clearance':.15,'required_length':70,'allowed_contact_parts':allowed})
    for label,shaft in [('left front hub','front axle'),('right front hub','front axle'),('left drum bearing','rear drum axle'),('right drum bearing','rear drum axle')]:
        parameters['connections'].append({'type':'shaft_hole','shaft':shaft,'part':label,'hole':'center-hole','fit':'bearing'})
