...
class PLTProtocolTests(unittest.TestCase):
 def test_mcp_schema_and_calls(self):
  ...
  with patch('plans.gate_job',return_value={'ok':True}),patch('payas_cad._save_build',side_effect=save),TestClient(app) as client:
   ...
   self.assertIn('repair_cut_gaps',design['inputSchema']['properties'])
   self.assertIn('repair_cut_gaps=true',design['description'])
   ...
   import json
   self.assertIn('holding_nicks',design['inputSchema']['properties']);self.assertIn('surface_texts',design['inputSchema']['properties'])
   question=client.post('/mcp',headers=headers,json={'jsonrpc':'2.0','id':99,'method':'tools/call','params':{'name':'create_design','arguments':{'primitives':[{'type':'panel','w':100,'h':80}]}}})
   asked=json.loads(question.json()['result']['content'][0]['text']);self.assertEqual(asked['status'],'NEEDS_INPUT');self.assertEqual(len(asked['questions']),2)
   ...
   profile_data=json.loads(profile.json()['result']['content'][0]['text'])
   self.assertFalse(profile_data['operations']['ENGRAVE']['through_cut'])
   self.assertLess(profile_data['operations']['ENGRAVE']['power_scale'],profile_data['operations']['CUT']['power_scale'])
   ...
   for i,source,expected in [(3,'IN;SP1;PU0,0;PD400,0,400,400,0,400,0,0;PU;',True),(4,'PU0,0;PD400,0;PE;',False)]:
    response=client.post('/mcp',headers=headers,json={'jsonrpc':'2.0','id':i,'method':'tools/call','params':{'name':'create_design','arguments':{'plt':source,'holding_nicks':False,'surface_texts':[]}}})
   ...
   response=client.post('/mcp',headers=headers,json={'jsonrpc':'2.0','id':5,'method':'tools/call','params':{'name':'create_design','arguments':{'svg':'<svg width="10mm" height="10mm" viewBox="0 0 10 10"><path d="M0 0 L2.7 0 L2.7 9 L0 9 Z"/></svg>','parameters':{'svg_default_operation':'CUT','thickness':2.7,'holding_nicks':False,'surface_texts':[]}}}})
   self.assertEqual(response.status_code,200)
   ...
   response=client.post('/mcp',headers=headers,json={'jsonrpc':'2.0','id':7,'method':'tools/call','params':{'name':'create_design','arguments':{'preset':'engraving_layout','parameters':{'holding_nicks':False,'surface_texts':[],'width_mm':100,'height_mm':100,'items':[{'kind':'text','value':'ÖNCE GÜVENLİK','x':50,'y':50,'width':80}]}}}})
   self.assertEqual(response.status_code,200)
   data=json.loads(response.json()['result']['content'][0]['text'])
   self.assertTrue(data['success'])
   self.assertEqual(data['method'],'positioned_vector_layout')
   from tests.test_image_engraving import sample
   response=client.post('/mcp',headers=headers,json={'jsonrpc':'2.0','id':8,'method':'tools/call','params':{'name':'create_from_reference','arguments':{'image_base64':sample('white','black'),'format':'both','parameters':{'holding_nicks':False,'surface_texts':[]},'primitives':[{'type':'panel','w':100,'h':100,'edges':'eeee','label':'top-panel'}],'reference_markings':[{'target_part':'top-panel','x':50,'y':70,'width':25}]}}})
   self.assertEqual(response.status_code,200)
