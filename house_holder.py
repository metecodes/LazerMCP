...
def recipe(width=130,depth=90,height=180,thickness=3):
    ...
    if w<100 or d<60 or h<140 or t!=3:
        raise ValueError('Reference recipe requires width>=100, depth>=60, height>=140 and 3 mm material.')
    ...
    slots=[slot(1.5*t,mid,t,span,'left-side','front'),slot(w-1.5*t,mid,t,span,'right-side','front'),slot(w/2,6.5,14,t,'floor','front'),slot(w/2,20,t,16,'divider','front')]
    ...
    for name,x in [('left-side',t),('right-side',w-2*t)]:
        sides.append({'type':'contour','label':name,'points':sidepts,'placement':pose([x,t,0],[0,1,0],[0,0,1]),'tabs':[tab('front',-t/2,mid,t,span),tab('back',length+t/2,mid,t,span)],'slots':[slot(length/2,6.5,22,t,'floor','left' if name=='left-side' else 'right')]})
    ...
    divider={'type':'contour','label':'divider','points':dividerpts,'placement':pose([w/2-t/2,t,8],[0,1,0],[0,0,1]),'tabs':[tab('front',-t/2,12,t,16),tab('back',length+t/2,12,t,16),tab('bottom',cy,-t/2,22,t)],'holes':[{'x':cy,'y':dh*.65,'d':30}]}
    front.pop('markings')
