...
mcp = MCPServer(
    ...
    instructions=(
        ...
        "A structural design derived from a photo can never receive PROTOTYPE READY from geometric validity alone. parameters.reference_job=true requires reference_parts:[{reference_part,generated_part,role,expected_outer_shape,silhouette_points?}]. The reviewer must prove one-to-one part mapping, structural OUTER_CUT fidelity, geometric tab-slot alignment, an assembled_preview.svg, nominal hardware fit and motion clearance. For moving parts pass hardware_fits:[{part,diameter_mm,tolerance_mm}] and motion:{drive_type:'direct_motor_shaft',moving_part, motor_axis:{origin:[x,y,z],direction:[x,y,z]},propeller_center:[x,y,z],radius_mm,clearance_mm,allowed_contact_parts:[]}; 360-degree clearance is sampled against explicitly placed panel geometry. Labels, mate metadata, ENGRAVE silhouettes and prose are not physical evidence. A custom reference silhouette mapped to a rectangle is FAIL. "
