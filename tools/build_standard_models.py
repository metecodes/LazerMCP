...
Original path segments (including source defects) are retained exactly.
"""
import hashlib
import json
import sys
...
def remove_source_nicks(data):
    ...
    for start,p in enumerate(paths):
        ...
        while True:
            ...
            if abs(q.end-end)>1e-8: segments.append(Line(q.end,end))
            if nxt==(start,0): break
            current=nxt
        loops.append(SVGPath(*segments))
...
def build():
    ...
    manifest={'id':MODEL_ID,'version':2,'title':'Ev Kalemlik','source_type':'user_supplied_svg',
              ...
              'note':'Kullanıcının SVG çizimi korunmuştur. Yalnız uçlar arasındaki noç boşlukları kapatıldı. Ölçek, geçmeler ve mevcut çizgiler değiştirilmedi. Kaynaktaki mikro taşmalar korunmuştur; montaj doğrulaması yapılmadı.'}
