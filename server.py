...
mcp = MCPServer(
    ...
    instructions=(
        ...
        "A structural design derived from a photo can never receive PROTOTYPE READY from geometric validity alone. parameters.reference_job=true requires reference_parts:[{reference_part,generated_part,role,expected_outer_shape,silhouette_points?}]. The reviewer must prove one-to-one part mapping, structural OUTER_CUT fidelity, geometric tab-slot alignment, an assembled_preview.svg, nominal hardware fit and motion clearance. panel.tabs metadata is never geometry: every tab rectangle must occur on the panel's actual outer CUT boundary and is transformed into the receiving panel's 3D frame before slot fit can PASS. For a directly driven rotor pass connections:[{type:'direct_motor_shaft',motor_part,driven_part,shaft_axis:{origin:[x,y,z],direction:[x,y,z]},driven_center:[x,y,z],radius_mm,clearance_mm,allowed_contact_parts:[]}]. A direct_motor_shaft connection verifies named endpoints, coaxiality and 360-degree clearance and does not require two opposite wall holes. Labels, mate metadata, ENGRAVE silhouettes and prose are not physical evidence. A custom reference silhouette mapped to a rectangle is FAIL. "
