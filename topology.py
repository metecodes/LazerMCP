...
def inspect_topology(svg_bytes: bytes | None) -> dict[str, Any]:
    ...
    for el in root.iter():
        ...
        for sub_index,contour in enumerate(subs,1):
            ...
            if gap > _NICK_GAP:
                open_cuts.append({"path": index,"subpath":sub_index,"gap_mm": round(float(gap), 3),"note": "CUT loop does not close (not a holding nick)."})
                continue
            if gap > 0.05:nicked += 1
            closed += 1;points = [contour.start]
            for segment in contour:
                if abs(segment.start-segment.end)<1e-9 and isinstance(segment,Line):continue
                forward=tuple(round(v,5) for p in segment.bpoints() for v in (p.real,p.imag));backward=tuple(round(v,5) for p in reversed(segment.bpoints()) for v in (p.real,p.imag));key=min(forward,backward)
                if key in seen:duplicates.append({"path":index,"subpath":sub_index,"note":"Duplicate CUT segment."})
                else:seen.add(key)
                n=1 if isinstance(segment,Line) else max(8,min(2048,int(segment.length(error=1e-5)/.2)+1))
                points.extend(segment.point(i/n) for i in range(1,n+1))
            if len(points)<4:continue
            points[-1]=points[0]
            try:poly=Polygon([(p.real,p.imag) for p in points])
            except Exception:continue
            if poly.area<=0.05:continue
            if not poly.is_valid:
                reason=explain_validity(poly)
                if "self-intersection" in reason.lower():self_intersections.append({"path":index,"subpath":sub_index,"note":reason})
    return {
