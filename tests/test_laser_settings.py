...
class LaserSettingsTests(unittest.TestCase):
 def test_engrave_is_single_low_power_surface_pass(self):
  ...
  self.assertEqual(engrave['speed_scale']*30,60);self.assertEqual(engrave['power_scale']*50,15);self.assertLess(engrave['power_scale'],cut['power_scale']);self.assertEqual(engrave['passes'],1);self.assertFalse(engrave['through_cut'])
